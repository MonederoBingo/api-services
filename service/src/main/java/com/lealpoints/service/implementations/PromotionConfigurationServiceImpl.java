...
public class PromotionConfigurationServiceImpl extends BaseServiceImpl implements PromotionConfigurationService {
    ...
    public ServiceResult<Long> insert(PromotionConfiguration promotionConfiguration) {
        try {
            ...
            return new ServiceResult<>(true, getTranslation(Message.PROMOTION_SUCCESSFULLY_ADDED), promotionConfigurationId);
        } catch (Exception ex) {
            logger.error(ex.getMessage(), ex);
            return new ServiceResult<>(false, getTranslation(Message.COMMON_USER_ERROR), null);
        }
    ...
    public ServiceResult<List<PromotionConfiguration>> getByCompanyIdRequiredPoints(long companyId, final String phone) {
        try {
            ...
            if (client == null) {
                return new ServiceResult<>(false, getTranslation(Message.PHONE_NUMBER_DOES_NOT_EXIST));
            }
            ...
            if (resultPromotionConfigurations.isEmpty()) {
                message = getTranslation(Message.CLIENT_DOES_NOT_HAVE_AVAILABLE_PROMOTIONS);
            }
    ...
    public ServiceResult<Boolean> deletePromotionConfiguration(long promotionConfigurationId) {
        try {
            ...
            if (deletedRows > 0) {
                String message = getTranslation(Message.THE_PROMOTION_WAS_DELETED);
                return new ServiceResult<>(true, message, true);
            } else {
                String message = getTranslation(Message.THE_PROMOTION_COULD_NOT_BE_DELETED);
